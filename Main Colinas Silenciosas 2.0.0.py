...
while True:
    ...
    while True:
          dni = input(f"{Fore.CYAN}DNI (formato 12345678A):{Style.RESET_ALL} ").strip().upper()
          if len(dni) == 9 and dni[:8].isdigit() and dni[8].isalpha():
              break
          else:
              print(f"{Fore.RED}DNI no válido. Debe tener 8 números y una letra.{Style.RESET_ALL}")
              input()
              os.system("cls")
...
while True:
            nombre_prota  = input(f"{Fore.CYAN}Para comenzar esta aventura, elige el nombre del protagonista y presiona ENTER para continuar: {Style.RESET_ALL} ").strip()
            if nombre_prota .replace(" ", "").isalpha():
                break
            else:
                print(f"{Fore.RED}El nombre solo debe contener letras.{Style.RESET_ALL}")
                input()
                os.system("cls")
